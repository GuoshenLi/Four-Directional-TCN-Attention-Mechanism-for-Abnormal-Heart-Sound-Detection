...
def residual_block(X, out_channels, stride, training):
    ...
    nb_first_layer_filter = out_channels // 2
    width = nb_first_layer_filter // cardinality
    X_shortcut = X
    sum_branch = tf.zeros([])
    ...
    for i in range(cardinality):
        ...
        X_ = tf.layers.batch_normalization(X, axis=-1, training=training)
        X_ = tf.nn.relu(X_)
        X_ = tf.layers.conv2d(X_, filters=width, kernel_size=(1, 1), strides=(1, 1), padding='same',
                             kernel_initializer=tf.initializers.glorot_normal())

        # Second component of Branch1
        X_ = tf.layers.batch_normalization(X_, axis=-1, training=training)
        X_ = tf.nn.relu(X_)
        X_ = tf.layers.conv2d(X_, filters=width, kernel_size=(3, 3), strides=(stride, stride), padding='same',
                             kernel_initializer=tf.initializers.glorot_normal())
        ...
        X_ = tf.layers.batch_normalization(X_, axis=-1, training=training)
        X_ = tf.nn.relu(X_)
        X_ = tf.layers.conv2d(X_, filters=out_channels, kernel_size=(1, 1), strides=(1, 1), padding='same',
                             kernel_initializer=tf.initializers.glorot_normal())
    ...
    sum_branch = sum_branch + X_shortcut
